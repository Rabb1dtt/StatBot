...
| AM-D | Дриблёр | `successfulDribbles`, `successfulDribblesPercentage`, `xA`, `wasFouled` | `progCarry%`, `possLost`, `progression` |
| W | Вингер | `successfulDribbles`, `xG`+`xA`, `keyPasses`, `shotsOnTarget`, `goals`+`assists` | `progCarry%`, `possLost`, `heatmap (фланг%)`, `sprints`, `topSpeed` |
